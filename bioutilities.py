    ...
    def extract_motifs(self,seq, report_mode='full',single_motif='',p_value=None,specific_strand=''):
        ...
        with tempfile.NamedTemporaryFile('w+',dir=self.temp_directory,delete=False) as tmp_file:
            ...
            if single_motif or p_value:
                ...
                command_to_run='fimo --verbosity 0 %s --skip-matched-sequence --thresh %f --bgfile %s %s' %   ( ('--motif %s' % single_motif) if single_motif else '', p_value if (p_value is not None) else self.p_value,self.bg_filename,self.meme_motifs_filename)
            else:
